...
Before we can start using the Twilio API, we will need to set-up our credentials. Create a file called `environment.js`. The `environment.js` file will store our API credentials in the `proccess.env object`. Please make sure to replace the two values below with your corresponding API credentials. 
...
We will need to require our `environment.js` file and the `twilio` module we installed earlier. 
...
With the `twilio` module and our credentials included in our code, we can start using Twilio's REST API.  We will start by creating a client.
...
When a text message is sent to our temporary phone number, Twilio will send that message over to our lambda function. It will provide us with three parameters need in our app. 

    To - Our Twilio phone number. 
    From - The phone number that sent us the message. 
    Body - The message that wa sent. 

If you want to experiment with other paramters sent by Twilio, you can find them by going to (https://www.twilio.com/docs/api/rest/sending-messages) 

**Note:** Even though the paramaters provided to us are capitalized, we are going to use them in the traditional camel-case form. In a later step, we will intercept the request from twilio and format the parameters. 
